...
将 `canary-alpha-mcp` 添加到 `mcpServers` 对象中。将 `<repo-path>` 替换为你的本地仓库绝对路径：
...
编辑完配置后重启 Claude Desktop，`canary-alpha-mcp` 的工具应出现在工具列表中。
...
**端口错误**：默认端口为 `18765`。如果在 `~/.canary-alpha-mcp/config.yaml` 中修改了端口，请相应更新 URL。
