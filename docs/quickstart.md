...
- write or update `~/.hermes/config.yaml` with the `canary-alpha-mcp` MCP server
...
Once `hermes mcp test canary-alpha-mcp` passes, try a bounded tool call through chat:

```bash
hermes chat --toolsets canary-alpha-mcp --query "Call recall with the last hour as the from/to window and granularity=session, then summarize."
```
