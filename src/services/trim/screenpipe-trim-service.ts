...
export async function runTrimOnce(databasePath: string, options?: TrimOptions): Promise<ScreenpipeTrimResult> {
  ...
  let retention: RetentionResult = { framesDeleted: 0, elementsDeleted: 0, reachedFloor: false };

  if (options?.budgetBytes !== undefined) {
    retention = await runRetentionIfOverBudget(
      databasePath,
  ...
  return { ...retention, durationMs: Date.now() - start };
}
