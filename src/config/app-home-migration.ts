...
import { isLegacyManagedServiceLoaded, LEGACY_LAUNCHD_LABEL } from './legacy-managed-service.js';
...
async function assertNoActiveLegacyRuntimeWriters(legacyDirectory: string): Promise<void> {
  const writerRoots = new Set<string>([legacyDirectory]);
  ...
  for (const root of writerRoots) {
    ...
    if (existsSync(rebuildLockPath)) {
      try {
        const lock = JSON.parse(await readFile(rebuildLockPath, 'utf8')) as {
          pid?: unknown;
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const parsed = JSON.parse(raw) as { pid?: unknown; processStartedAt?: unknown };
...
export function resolveLegacyAppDirectory(homeDirectory = homedir()): string {
  return join(homeDirectory, LEGACY_APP_DIRECTORY_NAME);
}

export function resolveTargetAppDirectory(homeDirectory = homedir()): string {
 ...
 */
export function replaceLegacyAppHomePathSegment(
  text: string,
  from: string,
  to: string
): string {
...
export type AppHomeMigrationResult =
  | { status: 'skipped'; reason: 'legacy-missing'; targetDirectory: string }
  | { status: 'blocked'; reason: 'both-present'; legacyDirectory: string; targetDirectory: string }
  | { status: 'migrated'; legacyDirectory: string; targetDirectory: string; backupDirectory: string };

export async function rewriteLegacyAppHomePaths(
...
export function inspectAppHomeMigrationState(homeDirectory = homedir()): {
  ...
  status: 'ready' | 'legacy-only' | 'both-present' | 'missing';
} {
 ...
 *
 * - When both legacy and target directories exist, migration stops without
 *   overwriting or merging data.
 * - When only the legacy directory exists, it is copied to a timestamped backup
 *   and then renamed into place. Symlinked legacy homes are dereferenced so the
 *   backup contains recoverable file contents rather than another link.
 */
...
export async function ensureAppHomeReady(options: {
  ...
  const readyTargetDirectory = migration.targetDirectory;
  if (!existsSync(readyTargetDirectory)) {
    await mkdir(readyTargetDirectory, { recursive: true, mode: 0o700 });
  } else if (options.ensurePrivateMode) {
    await stat(readyTargetDirectory);
  }
