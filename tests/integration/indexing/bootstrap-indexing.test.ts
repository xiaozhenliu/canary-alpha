...
describe('app bootstrap indexing catch-up', () => {
  ...
  it('starts bootstrap indexing in the background without blocking app creation', async () => {
    ...
    const checkpointPath = join(homeDir, '.computer-history-mcp', 'retrieval-checkpoint.screenpipe.json');
  ...
  it('continues indexing new records after startup on the polling interval', async () => {
    ...
    await waitFor(async () => {
      const checkpoint = JSON.parse(
        await readCheckpoint(join(homeDir, '.computer-history-mcp', 'retrieval-checkpoint.screenpipe.json'))
      ) as { cursor: string };
